...
import '../Widgets/login_botton.dart';
import '../Widgets/tField.dart';
import '../providers/general_provider.dart';
import '../services/auth_services.dart';

class MobileLoginSignUpBody extends StatefulWidget {
  const MobileLoginSignUpBody({super.key});

  @override
  State<MobileLoginSignUpBody> createState() => _MobileLoginSignUpBodyState();
}

class _MobileLoginSignUpBodyState extends State<MobileLoginSignUpBody> {
  bool obscure = false;
  ...
  final defaultCryptos = [
    {'name': 'Bitcoin', 'symbol': 'BTC', 'value': 0.0},
    {'name': 'Ethereum', 'symbol': 'ETH', 'value': 0.0},
    {'name': 'Solana', 'symbol': 'SOL', 'value': 0.0},
    {'name': 'Cardano', 'symbol': 'ADA', 'value': 0.55},
    {'name': 'Polkadot', 'symbol': 'DOT', 'value': 0.00},
    {'name': 'Litecoin', 'symbol': 'LTC', 'value': 0.0},
    {'name': 'Ripple', 'symbol': 'XRP', 'value': 0.0},
    {'name': 'Avalanche', 'symbol': 'AVAX', 'value': 0.0},
    {'name': 'Chainlink', 'symbol': 'LINK', 'value': 0.0},
    {'name': 'Dogecoin', 'symbol': 'DOGE', 'value': 0.14},
  ];
  Future<void> createUserCryptos(String userId) async {
    final userCryptosRef = FirebaseFirestore.instance
        .collection('users')
        .doc(userId)
        .collection('cryptos');

    for (var crypto in defaultCryptos) {
      await userCryptosRef.doc(crypto['symbol'] as String?).set(crypto);
    }
  }
